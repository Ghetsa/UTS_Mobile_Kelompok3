...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void initState() {
    super.initState();
    _loadRumahFromFirestore();
  }

  Future<void> _loadRumahFromFirestore() async {
    final snapshot = await FirebaseFirestore.instance.collection('rumah').get();
    setState(() {
      _rumahList = snapshot.docs
          .map((doc) => {
                'id': doc.id,
                'alamat': doc['alamat'] ?? 'Alamat kosong',
              })
          .toList();
      _loadingRumah = false;
    });
  ...
  Widget _buildFileUploadField({required String label}) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label,
              style: TextStyle(
  ...
  Widget _buildAddressSelectionField() {
    if (_loadingRumah) return const Center(child: CircularProgressIndicator());

    List<String> addressOptions =
        _rumahList.map((r) => r['alamat'] as String).toList();
    addressOptions.add(manualAddressOption);
    ...
    return Column(
      ...
      children: [
        _buildDropdownField(
          ...
          onChanged: (newValue) =>
              setState(() => _selectedAddressOption = newValue),
        ),
        if (showManualInput)
          _buildInputField(
          ...
          children: [
            const Text('Daftar Akun',
                ...
                style: TextStyle(
                    ...
                    color: AppTheme.primaryBlue)),
            const SizedBox(height: 6),
            const Text('Lengkapi formulir untuk membuat akun',
                textAlign: TextAlign.center,
